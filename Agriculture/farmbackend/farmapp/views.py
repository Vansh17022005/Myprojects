...
def signup(request):
    if request.method == 'POST':
        ...
        USER.objects.create(name=name,ph_number=number,email=email,password=password)
...
def del_data(request):
    ...
    if request.method=='GET':
        ...
        #USER.objects.filter(name='vansh').delete() 
        return True
    return True
...
def up_images(request):
    if request.method == 'POST':
        try:
            ...
            img_model = PRODUCT(name=name, image=image, price=price, category=cat)
            img_model.save()
...
def get_images(request):
    if request.method == 'POST':
        cat = request.POST.get('category')
        images = PRODUCT.objects.filter(category=cat)
        image_data = [
            {
                'id': img.id,
                'name': img.name,
...
def admin_login(request):
    if request.method=='POST':
        ...
        email1 = data.get('email')
        password1 = data.get('password')
        e1= serializers.serialize('json', ADMIN.objects.filter(email=email1,password=password1))
        if e1 is not None:
            return JsonResponse(e1,safe=False)
        else:
            return JsonResponse(e1,safe=False)
